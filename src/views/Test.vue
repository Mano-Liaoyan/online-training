    ...
    <v-container>
        <v-row justify="center">
            <v-col cols="12" sm="12" md="6" xl="6" lg="6">
                <v-img v-show="!done" lazy-src="../assets/test.jpg" :src="question_img_url"></v-img>
            ...
            <v-col sm="12" md="4" xl="4" lg="4">
                ...
                <v-radio-group v-model="selected_value">
                    <v-radio v-for="(option, i) in options" :key="i" :color="color"
                             :label="option.label" :value="option.value" :readonly="done">
                    </v-radio>
                </v-radio-group>
                <v-divider/>
                <p v-show="done">答案是{{ answer_value }}</p>
                <v-btn @click="get_question_id();selected_value = '';done = !done;" v-show="done">
                    next
                </v-btn>
            </v-col>
        </v-row>
